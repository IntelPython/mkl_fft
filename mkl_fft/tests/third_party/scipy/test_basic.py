...
class TestFFT:
    ...
    @skip_xp_backends(
        np_only=True, reason="significant overhead for some backends"
    )
    ...
    def _check_axes(self, op, xp):
        ...
        for a in axes:
            op_tr = op(permute_dims(x, axes=a))
            tr_op = permute_dims(op(x, axes=a), axes=a)
    ...
    def test_axes_subset_with_shape_standard(self, op, xp):
        ...
        for a in axes:
            ...
            op_tr = op(permute_dims(x, axes=a), s=shape[:2], axes=(0, 1))
            tr_op = permute_dims(op(x, s=shape[:2], axes=a[:2]), axes=a)
            xp_assert_close(op_tr, tr_op)
        ...
        for forw, back in func_pairs:
            ...
            for n in [xp_size(x), 2 * xp_size(x)]:
                for norm in ["backward", "ortho", "forward"]:
                    ...
                    xp_assert_close(np.linalg.norm(tmp, ord=2), x_norm)

    @pytest.mark.skip("float16 and longdouble are not supported")
    ...
    def test_dtypes_complex(self, dtype, xp):
        ...
        res_fft = fft.ifft(fft.fft(x))
        # Check both numerical results and exact dtype matches
        xp_assert_close(res_fft, x, rtol=1e-06, atol=1e-06)
...
def test_fft_with_order(dtype, order, fft, xp):
    ...
    if fft.__name__.endswith("fft"):
        ...
        for ax in axes:
            X_res = fft(X, axes=ax)
            Y_res = fft(Y, axes=ax)
            assert_array_almost_equal(X_res, Y_res, decimal=4)
    else:
        raise ValueError
