...
`fft(x, n=None, axis=-1, norm=None, out=out)` - 1D FFT, similar to `numpy.fft.fft`

`fft2(x, s=None, axes=(-2, -1), norm=None, out=out)` - 2D FFT, similar to `numpy.fft.fft2`

`fftn(x, s=None, axes=None, norm=None, out=out)` - ND FFT, similar to `numpy.fft.fftn`
